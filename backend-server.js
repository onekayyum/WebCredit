...
app.post("/api/backend/:method", (req, res) => {
  ...
  try {
    if (method === "addCustomer") {
      ...
      if (db.prepare("SELECT id FROM customers WHERE mobile = ?").get(mobile)) return res.json(null);
      ...
      try {
        db.prepare(
          "INSERT INTO customers (id, name, mobile, createdAt) VALUES (?, ?, ?, ?)",
        ).run(id, name, mobile, createdAt);
      } catch (error) {
        const message = String(error);
        if (message.includes("UNIQUE constraint failed: customers.mobile")) {
    ...
    if (method === "updateCustomer") {
      ...
      if (db.prepare("SELECT id FROM customers WHERE mobile = ? AND id != ?").get(mobile, Number(b.id))) return res.json(false);

      db.prepare("UPDATE customers SET name = ?, mobile = ? WHERE id = ?").run(name, mobile, Number(b.id));
